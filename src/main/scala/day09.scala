    ...
    for(lineValueArrayIdx <- 0 to lineValueArrays.length-1){
      ...
      def suggestNext(resultingTree:Vector[Vector[Long]],tempWorkingTree:Vector[Vector[Long]]):Vector[Vector[Long]]={
        tempWorkingTree match {
          case _ if( 1==tempWorkingTree.length ) => tempWorkingTree++resultingTree
          case restWorkingTree :+ newWorkingTreeLast :+ lastVector => {
            var newerCopy = newWorkingTreeLast :+ (newWorkingTreeLast.last + lastVector.last)
            suggestNext(lastVector +: restWorkingTree, (restWorkingTree :+ newerCopy))
          }
        }
      }

      def suggestPrev(resultingTree:Vector[Vector[Long]],tempWorkingTree:Vector[Vector[Long]]):Vector[Vector[Long]]={
        tempWorkingTree match {
          case _ if( 1==tempWorkingTree.length ) => tempWorkingTree++resultingTree
          case restWorkingTree :+ newWorkingTreeLast :+ lastVector => {
            var newerCopy = (newWorkingTreeLast.head - lastVector.head)+:newWorkingTreeLast
            suggestPrev(lastVector +: restWorkingTree, (restWorkingTree :+ newerCopy))
          }
        }
      }

      // then loop up through for suggesting
      var suggestedTree = suggestPrev(Vector[Vector[Long]](), newCurrLineTree)

      // then we got through and add up the end values?

      if(includeDebuggingInfo) for(lineToPrint <- suggestedTree){
        vecPrinter("                  ", lineToPrint)
      }
      

      printf("adding [%d]: %d\n",lineValueArrayIdx, suggestedTree.head.head)
      accumulatedTotal = accumulatedTotal + suggestedTree.head.head
